...
describe("nextIndexForDirection", () => {
  ...
  it("moves up within bounds", () => {
    expect(nextIndexForDirection(5, null, 2, "up", vi.fn<(slotIndex: number) => void>())).toBe(1);
  });

  it("returns null when moving past the top", () => {
    const onForbidden = vi.fn<(slotIndex: number) => void>();
  ...
  it("returns null when moving past the bottom", () => {
    expect(
      nextIndexForDirection(5, null, 4, "down", vi.fn<(slotIndex: number) => void>()),
    ).toBeNull();
  });
  ...
  it("skips the turn player slot moving up", () => {
    expect(nextIndexForDirection(5, 2, 3, "up", vi.fn<(slotIndex: number) => void>())).toBe(1);
  });
